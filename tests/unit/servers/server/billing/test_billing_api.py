...
def test_circuit_starts_closed() -> None:
    from src.internal.servers.billing.api import _is_billing_circuit_open

    assert _is_billing_circuit_open() is False


def test_open_and_close_circuit() -> None:
    from src.internal.servers.billing.api import (
...
def test_billing_info_returns_not_subscribed_without_license(tmp_path):
    ...
    with patch(
        "src.internal.servers.billing.api.load_stored_license", return_value=None
    ):
...
def test_billing_info_503_when_circuit_open(tmp_path):
    from src.internal.servers.billing.api import _close_billing_circuit

    client, store = make_test_client(tmp_path)
    with (
        patch(
            "src.internal.servers.billing.api.load_stored_license",
            return_value="license_blob",
        ),
        patch(
            "src.internal.servers.billing.api._is_billing_circuit_open",
            return_value=True,
        ),
    ):
        resp = client.get("/admin/billing/billing-information", headers=_bearer(_ADMIN))
...
def test_billing_info_opens_circuit_on_502(tmp_path):
    ...
    with (
        ...
        patch(
            "src.internal.servers.billing.api._get_billing", new_callable=AsyncMock
        ) as mock_svc,
    ):
...
def test_stripe_key_from_override(tmp_path):
    ...
    import src.internal.servers.billing.api as billing_mod
