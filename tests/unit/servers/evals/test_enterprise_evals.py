...
from src.internal.servers.enterprise_settings.models import NavigationItem
from src.internal.servers.enterprise_settings.store import load_runtime_settings
...
def test_eval_run_with_fake_search(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.internal.servers.evals.api.run_expanded_search", fake_run_expanded_search
    )
    client, store = _make_client(tmp_path)
    resp = client.post(
        "/evals/eval_run",
