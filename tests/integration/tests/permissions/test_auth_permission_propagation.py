...
def test_basic_permission_granted_on_registration(
    ...
    assert "basic" in admin_perms  # noqa: F821,F841

    # Basic user should have 'basic' from Basic default group
    assert "basic" in basic_perms  # noqa: F821,F841
...
def test_role_downgrade_removes_basic_group_and_permission(
    ...
    # --- EXT_PERM_USER ---
    assert ext_user.email in _get_basic_group_member_emails(admin_user)  # noqa: F821,F841

    UserManager.set_role(
        user_to_set=ext_user,  # noqa: F821,F841
        target_role=UserRole.EXT_PERM_USER,
        user_performing_action=admin_user,
        explicit_override=True,
    )
    assert ext_user.email not in _get_basic_group_member_emails(admin_user)  # noqa: F821,F841

    # --- SLACK_USER ---
    assert slack_user.email in _get_basic_group_member_emails(admin_user)  # noqa: F821,F841

    UserManager.set_role(
        user_to_set=slack_user,  # noqa: F821,F841
        target_role=UserRole.SLACK_USER,
        user_performing_action=admin_user,
        explicit_override=True,
    )
    assert slack_user.email not in _get_basic_group_member_emails(admin_user)  # noqa: F821,F841
