...
All nine tasks are implemented on `refactor/grpo-minimal-architecture`. Three
places where the code contradicted this plan, corrected during execution:

1. **`compute_grpo_outcome_advantage` collides.** The plan's Task 3 lists it
   under `ALGORITHM_EXPORTS` and asserts each package *and* root export resolves
   to `algorithms.<name>`. Those two exports were different functions — the
   package re-exported the tensor form, the root re-exported the list form — so
   the assertion as written is unsatisfiable. `algorithms.py` now owns one
   dispatching entry point with an overload per call shape. See the spec.

2. **`plot_rollouts.py` was deleted without being relocated** by the Task 4
   commit, and its CLI takes `--jsonl/--out`, not the positional
   `INPUT --output OUTPUT` Task 5 describes. Task 5 restores the file at
   `examples/plot_grpo_rollouts.py` with its real interface.

3. **`torch.inference_mode()` for the frozen reference does not pay** (Task 8,
   Steps 3-4). It is correct, but it is 7-12% *slower* at realistic hidden and
   vocabulary sizes; it wins only on toy models. It was reverted and the
   measurement recorded. The contract test asserts the reference runs with grad
   disabled rather than pinning the mechanism.

Also corrected: the benchmark harness originally timed its samples with
`tracemalloc` active, inflating every median roughly three-fold. Timing and
allocation are now separate passes, and the first baseline was discarded.
...
- [x] **Step 1: Replace the current inventory expectation with the final target**
...
- [x] **Step 2: Add ownership and removed-path assertions**
...
- [x] **Step 3: Add an AST dependency-direction test**
...
- [x] **Step 4: Run the boundary tests and verify RED**
...
- [x] **Step 1: Write token-alignment and shared-identity tests**
...
- [x] **Step 2: Run the shared-helper tests and verify RED**
...
- [x] **Step 3: Move the helper without changing arithmetic**
...
- [x] **Step 4: Redirect DPO and GRPO while retaining the lazy GRPO export**
...
- [x] **Step 5: Run shared, DPO, and GRPO log-prob tests**
...
- [x] **Step 1: Extend lazy-export identity tests for every moved public symbol**
...
- [x] **Step 3: Build `algorithms.py` in dependency order**
...
- [x] **Step 4: Redirect imports and lazy registries**
...
- [x] **Step 5: Delete the three superseded modules**

Delete `core_algos.py`, `rollouts.py`, and `judge.py` with `apply_patch`. Do not leave re-export stubs.

- [x] **Step 6: Run the complete algorithm/reward/judge suite**
...
- [x] **Step 7: Check stale paths and commit**
...
- [x] **Step 1: Add final trainer-ownership and no-cycle tests**
...
- [x] **Step 3: Move the trainer hierarchy into `training.py`**
...
- [x] **Step 4: Invert the controller dependency without duplicating orchestration**
...
- [x] **Step 5: Redirect consumers and delete `trainers.py`**
...
- [x] **Step 6: Run trainer, controller, generation, DPO, and cycle tests**
...
- [x] **Step 1: Add relocated-module and CLI-output tests**
...
- [x] **Step 2: Run the plotting and inventory tests and verify RED**
...
- [x] **Step 3: Move the CLI implementation unchanged**
...
- [x] **Step 4: Delete the package CLI and run tests**
...
- [x] **Step 1: Write failing benchmark-harness tests**
...
- [x] **Step 3: Implement a deterministic diagnostic harness**
...
- [x] **Step 4: Record the pre-optimization baseline**
...
- [x] **Step 1: Write equivalence and invocation-count tests**
...
- [x] **Step 3: Add the canonical group kernel and redirect adapters**
...
- [x] **Step 4: Consolidate correctness and sparse-vector work**
...
- [x] **Step 5: Skip provably unused scalar-path components**
...
- [x] **Step 6: Run reward and GRPO numerical suites**
...
- [x] **Step 7: Rerun benchmarks and retain only evidenced improvements**
...
- [x] **Step 1: Add model-forward, inference-mode, and tensor-equivalence contracts**
...
- [x] **Step 2: Add concurrency and ordering contracts**
...
- [x] **Step 3: Run the new contracts and verify RED**
...
- [x] **Step 4: Optimize measured policy/reference inference**
...
- [x] **Step 5: Optimize measured batch assembly**
...
- [x] **Step 6: Optimize measured rollout scheduling**
...
- [x] **Step 7: Run focused behavioral suites**
...
- [x] **Step 8: Rerun benchmarks and retain only evidenced improvements**
...
- [x] **Step 2: Run exact stale-path and inventory checks**
...
- [x] **Step 5: Commit documentation and final contracts**
...
- [x] **Step 6: Request a code review against the complete range**
...
- [x] **Step 7: Create the delivery branch and PR**
