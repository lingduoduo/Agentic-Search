...
| `AGENTIC_SEARCH_INTENT_MIN_ROUTE_MARGIN` | Minimum cosine-similarity gap between the top and runner-up route; defaults to `0.02`, which under `e5-small-v2` abstains on roughly two thirds of requests |
...
| `AGENTIC_SEARCH_INTENT_MIN_ROUTE_MARGIN` | `0.02` | Minimum cosine-similarity gap between the top and runner-up route; must be finite and between `0.0` and `1.0`. MiniLM-era value: e5's in-scope margins have a median of `0.0129`, so this abstains on 73 of 111 test-slice requests |
...
| `AGENTIC_SEARCH_INTENT_TOP_K` | `3` | Neighbors averaged per route; must be a positive integer. Now swept on a tuning slice rather than read off a table, but the e5 sweep selected nothing (no combination cleared its coverage floor), so the shipped default is unchanged — see [Training and evaluation](training-and-evaluation.md) |
