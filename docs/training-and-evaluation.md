...
**What the current model actually scores, and why it is not promotable.** On the committed dataset at seed 17, realistic accuracy is `0.567` and the out-of-scope separation margin (mean in-scope confidence minus mean out-of-scope confidence) is `+0.071`. Both are pinned by tests in `tests/unit/test_intent_training.py`. The accuracy does not beat the `3/5 = 0.60` hand-scored baseline that motivated this work. The cause is measured and structural: frame-based generation grew the learned vocabulary only from 126 to 147 tokens, and the hand-authored evaluation queries are just **47%** in-vocabulary — the other half is ordinary English the frames never contain (*dashboard, postmortem, checklist, spreadsheet, standup, recall, latency, whether, anything*). A bag-of-embeddings model over a from-scratch vocabulary cannot read words it never saw. Closing that gap needs a representation change (a pretrained sentence encoder, or character n-grams), not more of the same synthetic data. The separation margin is the genuine improvement: out-of-scope probes now produce 17 distinct confidences across 24 probes, where previously every fully-out-of-vocabulary probe pooled to the zero vector and returned one identical prediction.

Two defaults were corrected against these measurements. `--epochs` defaults to `300`, not `10`: training takes one **full-batch** step per epoch, so 10 epochs is 10 optimizer steps and leaves the model effectively untrained (realistic accuracy `0.333`). `--min-freq` defaults to `1`: dropping singleton tokens to manufacture unknown words during training was measured to cost more vocabulary than the unknown-token signal it buys — it left only 4 of 341 training rows containing an unknown word, shrank the vocabulary to 135, and pushed the out-of-scope margin negative. The unknown-token embedding still matters at `--min-freq 1`, because its purpose is that an unread word maps to its own index instead of the padding id that masked pooling deletes.
