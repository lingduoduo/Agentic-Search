...
class OpenAPISchema(BaseModel):
    ...
    paths: dict[str, dict] = Field(
        default_factory=dict, validate_default=True, description="Path definitions"
    )
    ...
    def validate_server(cls, server: str) -> str:
        ...
        if server is None or server == "":
            raise ValueError("The 'server' field cannot be empty and must be a string")
        return server
    ...
    def validate_description(cls, description: str) -> str:
        ...
        if description is None or description == "":
            raise ValueError(
                "The 'description' field cannot be empty and must be a string"
            )
        return description
    ...
    def validate_paths(cls, paths: dict[str, dict]) -> dict[str, dict]:
        ...
        if not paths or not isinstance(paths, dict):
            raise ValueError("'paths' in openapi_schema must be a non-empty dictionary")
        ...
        for interface in interfaces:
            ...
            if not isinstance(operation.get("description"), str):
                raise ValueError(
                    "Each operation must have a non-empty 'description' string"
                )
            if not isinstance(operation.get("operationId"), str):
                raise ValueError(
                    "Each operation must have a non-empty 'operationId' string"
                )
            if not isinstance(operation.get("parameters", []), list):
                raise ValueError("'parameters' must be a list or omitted")

            # 5. Ensure operationId is unique
            if operation["operationId"] in operation_ids:
                raise ValueError(
                    f"'operationId' must be unique. Duplicate: {operation['operationId']}"
                )
