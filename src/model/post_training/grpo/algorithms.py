"""Grouped rollout sampling and scoring, judges, and on-policy batch assembly.

Torch-free on purpose, like ``reward.py``: the tensor-level losses live beside
this in ``core_algos.py``. Importing them here would put ``import torch`` in
front of the judges and the rollout scoring, and the CI unit-test job runs
