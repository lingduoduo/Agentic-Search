...
``score_prompt_batch`` in :mod:`src.training.grpo.rollouts`); a real LLM judge can
...
consumers are the GRPO example scripts -- so it was moved here from the shared
top level, next to the flow that uses it.

That choice accepts a known future cost, recorded here so it does not later read
as an oversight: a judge-backed DPO pair loader (the natural follow-up to the DPO
trainer, which was deliberately built source-agnostic) would make ``dpo`` import
``grpo``. That is a method package depending on another method package, the edge
the top-level ``data``/``reward`` modules exist to avoid. It was weighed and
accepted; if that loader is built and the edge bites, moving this module back up
one level is the fix.
