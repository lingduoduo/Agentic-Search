"""PPO: the clipped-surrogate algorithm layer that `rl` is built on.
...
surrogate with a group-relative advantage in place of GAE, so `rl` importing
`ppo` is a variant importing its base. `sft/`, `dpo/` and `rl/` are methods you
run; `ppo/` is arithmetic they run on, which is why nothing imports it *from*
`rl` in the other direction.

Re-exports are lazy for the same reason they are in `rl`: importing any
