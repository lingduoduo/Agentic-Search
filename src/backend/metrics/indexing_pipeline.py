...
# ---------------------------------------------------------------------------
# Celery/Redis stubs — replace with real implementations when Celery is wired in
# ---------------------------------------------------------------------------


def celery_get_broker_client(celery_app: Any) -> "Redis":  # type: ignore[type-arg]
    raise NotImplementedError("celery_get_broker_client not wired")


def celery_get_queue_length(queue_name: str, redis_client: "Redis") -> int:  # type: ignore[type-arg]
    raise NotImplementedError("celery_get_queue_length not wired")


def celery_get_unacked_task_ids(queue_name: str, redis_client: "Redis") -> list[str]:  # type: ignore[type-arg]
    raise NotImplementedError("celery_get_unacked_task_ids not wired")
...
class WorkerHealthCollector(_CachedCollector):
    ...
    def _collect_fresh(self) -> list[GaugeMetricFamily]:
        ...
        active_workers = GaugeMetricFamily(
            "agentic_search_celery_active_worker_count",
        ...
        worker_up = GaugeMetricFamily(
            "agentic_search_celery_worker_up",
