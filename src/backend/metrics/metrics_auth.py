...
DISABLE_METRICS_AUTH: bool = os.environ.get("DISABLE_METRICS_AUTH", "").lower() in (
    "true",
    "1",
    "yes",
)
METRICS_AUTH_TOKEN: str | None = os.environ.get("METRICS_AUTH_TOKEN") or None
...
def verify_metrics_token(request: Request) -> None:
    ...
    and any missing/invalid token raises HTTP 401. The token comparison is
    constant-time to avoid leaking it via response timing.
    """
