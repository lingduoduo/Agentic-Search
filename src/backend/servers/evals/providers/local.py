"""Local eval provider — runs evaluations and prints results to the CLI."""
...
class LocalEvalProvider(EvalProvider):
    ...
    def _run_multi_turn_eval(
        ...
        try:
            result = multi_turn_task({**input_data})
            ...
            print(
                f"\n  {BOLD}Multi-turn result:{RESET} {status} "
                f"({result.pass_count}/{result.total_turns} turns passed)"
            )
        except Exception as e:
            print(f"  {RED}ERROR:{RESET} {e}")
            failed[0] += 1
