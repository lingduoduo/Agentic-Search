...
from src.backend.cache.interface import CacheBackend
from src.backend.cache.interface import CacheLock
from src.backend.cache.interface import TTL_KEY_NOT_FOUND
from src.backend.cache.interface import TTL_NO_EXPIRY
...
def _get_session_with_tenant(tenant_id: str):  # type: ignore[return]
    try:
        from src.backend.db.engine.sql_engine import get_session_with_tenant  # type: ignore[import]

        return get_session_with_tenant(tenant_id=tenant_id)
    except ImportError:
        raise NotImplementedError(
            "PostgresCacheBackend requires src.backend.db.engine.sql_engine"
        )


def _get_session_with_current_tenant():  # type: ignore[return]
    try:
        from src.backend.db.engine.sql_engine import get_session_with_current_tenant  # type: ignore[import]
...
def _get_cache_store_model():  # type: ignore[return]
    try:
        from src.backend.db.models import CacheStore  # type: ignore[import]

        return CacheStore
    except ImportError:
        raise NotImplementedError(
            "PostgresCacheBackend requires src.backend.db.models.CacheStore"
        )
