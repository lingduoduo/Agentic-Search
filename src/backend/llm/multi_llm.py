...
from .request_context import get_llm_mock_response
from .utils import (
    build_litellm_passthrough_kwargs,
    is_true_openai_model,
    model_is_reasoning_model,
)
from .well_known_providers.constants import (
...
MOCK_LLM_RESPONSE: str | None = os.environ.get("MOCK_LLM_RESPONSE")
...
class LitellmLLM(LLM):
    ...
    def _safe_model_config(self) -> dict:
        ...
        dump["api_key"] = _mask_string(dump.get("api_key") or "")
        custom_config = dump.get("custom_config")
        if isinstance(custom_config, dict):
            masked_config = {}
            for k, v in custom_config.items():
                masked_config[k] = _mask_string(v) if v else v
            dump["custom_config"] = masked_config
        return dump

    def _track_llm_cost(self, usage: Usage) -> None:
        """Cost tracking stub — override in subclasses if needed."""
    ...
    def _completion(
        ...
        from .litellm_singleton import litellm
    ...
    def invoke(
        ...
        from .model_response import from_litellm_model_response
    ...
    def stream(
        ...
        from .model_response import from_litellm_model_response_stream
