"""Tenant-scoped cache for query embeddings.
...
Model identity comes from ``search_settings.id`` — every field that affects the
resulting vector is assumed immutable for a given id, so a model swap allocates a
new row + new id and old entries simply expire.
...
from src.backend.cache.interface import CACHE_TRANSIENT_ERRORS
from src.backend.cache.interface import get_cache_backend
from src.backend.document_index.utils import setup_logger
from src.backend.natural_language_processing._stubs import (
...
def get_cached_query_embeddings(
    ...
    Args:
        ...
        search_settings_id: The row ID of the search settings.
