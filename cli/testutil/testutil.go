...
// AgentSearchServer returns an httptest.Server that simulates the Agentic Search backend.
// Routes are mounted under /api to match the production URL layout.
func AgentSearchServer(meStatus int) *httptest.Server {
...
func IsolateConfig(t *testing.T, serverURL string) {
	...
	t.Setenv("AGENTIC_SEARCH_URL", serverURL)
	t.Setenv("AGENTIC_SEARCH_PAT", "test-key")
}
